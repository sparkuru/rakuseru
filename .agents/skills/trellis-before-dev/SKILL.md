...
7. **Check the dev-command wrapper**:
   - Prefer repo-local wrapper commands such as `./hako` for install, lint, typecheck, test, build, and dev-server commands.
   - If the repository has no wrapper and the task needs local toolchain commands, apply the `dev-it-in-docker` skill before implementation.
   - Do not broaden allow rules to raw Docker, shell, or package-manager commands just to make validation convenient.
